...
contract LifeEstatesMarketPlace is ERC1155Holder, Ownable, ReentrancyGuard {
...
event TokensBought(uint256[] indexed tokenIds, uint256[] amounts, uint256[] prices);
...
event TokensSold(uint256[] indexed tokenIds, uint256[] amounts, uint256[] prices);
...
//Functions
  ...
  function listToken(uint256 tokenId, uint256 amount, uint256 price, address newLifeEstate) public {
    ...
    require(propertyToSale.isApprovedForAll(msg.sender, address(this)), "Marketplace needs approval to manage tokens");
    require(propertyToSale.balanceOf(msg.sender, tokenId) >= amount, "Not enough tokens owned to list");
    
    //Create a new listing
    listings[listingCounter] = Listing(msg.sender, tokenId, amount, price, true, newLifeEstate);
    listingCounter += 1;
  ...
  function buyToken(uint256 listingID, address tokenAddress) public nonReentrant {
    ...
    LifeEstateNFT lifeEstateNFT = LifeEstateNFT(listing.newLifeEstate);
    lifeEstateNFT.safeTransferFrom(listing.seller, msg.sender, tokenId, amount, "");
